/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2022 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g0xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */
void maincpp();
/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define ZC_A_Pin GPIO_PIN_9
#define ZC_A_GPIO_Port GPIOB
#define ZC_A_EXTI_IRQn EXTI4_15_IRQn
#define ZC_C_Pin GPIO_PIN_14
#define ZC_C_GPIO_Port GPIOC
#define ZC_C_EXTI_IRQn EXTI4_15_IRQn
#define ZC_B_Pin GPIO_PIN_15
#define ZC_B_GPIO_Port GPIOC
#define ZC_B_EXTI_IRQn EXTI4_15_IRQn
#define BAT_VOLTS_Pin GPIO_PIN_2
#define BAT_VOLTS_GPIO_Port GPIOA
#define OD_C_Pin GPIO_PIN_6
#define OD_C_GPIO_Port GPIOA
#define PWM_C_Pin GPIO_PIN_7
#define PWM_C_GPIO_Port GPIOA
#define OD_B_Pin GPIO_PIN_0
#define OD_B_GPIO_Port GPIOB
#define PWM_B_Pin GPIO_PIN_1
#define PWM_B_GPIO_Port GPIOB
#define OD_A_Pin GPIO_PIN_2
#define OD_A_GPIO_Port GPIOB
#define REV_Pin GPIO_PIN_9
#define REV_GPIO_Port GPIOA
#define PWM_A_Pin GPIO_PIN_6
#define PWM_A_GPIO_Port GPIOC
#define signal_Pin GPIO_PIN_3
#define signal_GPIO_Port GPIOB
#define LED_Pin GPIO_PIN_4
#define LED_GPIO_Port GPIOB
#define Proshot_Pin GPIO_PIN_6
#define Proshot_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
